# 토폴로지 경로찾기 UI/UX 개선 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 토폴로지 모달의 경로 하이라이트를 도면 경로 trace와 시각 통일(실선·glow·포커스 디밍)하고, 경로찾기 선택을 쉽게 풀고 다시 고를 수 있게 인터랙션을 개선한다.

**Architecture:** 경로 하이라이트는 `animated` 점선을 실선+glow로 바꾸고, 경로 표시 중에는 경로 외 엣지·노드를 불투명도 0.2로 디밍한다(도면 `globalAlpha 0.2` 기법). 인터랙션은 선택된 엣드포인트 재클릭 시 해제, 빈 캔버스/ESC로 전체 해제, 컨트롤 바에 시작/종료 칩을 둔다. 레이아웃·base 그래프 파생은 그대로 — 디밍은 렌더 단계 오버레이.

**Tech Stack:** React 18 + TypeScript, `@xyflow/react` v12 (React Flow), Zustand, Tailwind CSS.

설계 문서: `docs/superpowers/specs/2026-05-22-topology-pathfinding-ux-design.md`
...
| 파일 | 변경 |
|---|---|
| `frontend/src/features/network/TopologyTestControls.tsx` (수정) | 시작/종료 칩 props + 렌더 |
| `frontend/src/features/network/NetworkTopologyModal.tsx` (수정) | path 스타일, 포커스 디밍, 인터랙션 재작성, 칩 연동 |

`FloatingEdge.tsx` 와 `pathfinding.ts` 는 변경 없음. 두 파일은 칩 props 계약으로 묶여 있어 (`tsconfig` `noUnusedLocals`/required props) 한 태스크에서 함께 교체해야 빌드가 통과한다.

모든 명령은 `frontend/` 디렉터리에서 실행한다.

---

## Task 1: 경로 하이라이트·인터랙션 개선

**Files:**
- Modify: `frontend/src/features/network/TopologyTestControls.tsx` (전체 교체)
- Modify: `frontend/src/features/network/NetworkTopologyModal.tsx` (전체 교체)

- [ ] **Step 1: TopologyTestControls.tsx 전체 교체**
...
- [ ] **Step 2: NetworkTopologyModal.tsx 전체 교체**
...
Expected: 성공 (`tsc -b && vite build` 통과). 실패 시 — `noUnusedLocals`/`noUnusedParameters` 위반 또는 재현 오타가 흔하므로 에러 메시지를 읽고 수정.

참고: `npm run lint` 는 리포에 ESLint v9 config 파일이 없어 프로젝트 전역에서 실패한다(기존 문제, 이 작업과 무관). 타입 검증은 `npm run build` 의 `tsc -b` 로 충분하다 — `npm run lint` 는 실행하지 않는다.

- [ ] **Step 4: 수동 검증**

개발 서버로 토폴로지 모달을 연다 (`docker compose -f docker-compose.dev.yml up -d` + `npm run dev`, 케이블 "상세" 클릭). 아래를 확인:

1. 시작·종료 노드를 클릭하면 경로가 **굵은 실선 + glow** 로 선명하게 표시되고, 경로 아닌 엣지·노드는 흐려진다(포커스 디밍).
2. 빈 캔버스를 클릭하면 경로찾기 선택이 풀리고 디밍이 해제된다. 끊은 경로·추가 경로는 유지된다.
3. ESC 를 누르면 경로찾기 선택이 풀린다(addMode 중이면 addMode 취소가 우선).
4. 시작·종료가 둘 다 선택된 상태에서 다른 노드를 클릭하면 아무 변화가 없다.
5. 선택된 시작점 노드를 다시 클릭하면 시작점만 해제된다(종료점 유지). 종료점도 동일.
6. 컨트롤 바에 "시작: <노드명>" / "종료: <노드명>" 칩이 뜨고, 칩의 × 가 해당 엣드포인트를 해제한다.
7. 노드 위치는 디밍·경로찾기 중에도 움직이지 않는다.

- [ ] **Step 5: 커밋**

```bash
git add src/features/network/TopologyTestControls.tsx src/features/network/NetworkTopologyModal.tsx
git commit -m "$(cat <<'EOF'
feat(network): 토폴로지 경로 하이라이트·경로찾기 UX 개선

경로를 실선+glow+포커스 디밍으로 도면 경로 trace 와 시각 통일. 빈 캔버스·ESC 로
경로찾기 해제, 선택된 엣드포인트 재클릭 시 개별 해제, 컨트롤 바에 시작/종료 칩 추가.
