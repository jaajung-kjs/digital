...
function asset(p: { id: string; code: string; name: string; parent?: string | null; sort?: number; floorId?: string | null }) {
  return {
    ...
    assetType: { id: p.code, code: p.code, name: p.code, group: null, role: p.code === 'DIST' ? 'panel' : p.code === 'FEEDER' ? 'feeder' : 'device', displayColor: null, fieldTemplate: [] },
...
vi.mock('../../../../assets/hooks/useAssetTypes', () => ({
  useAssetTypes: () => ({ data: [
    { id: 'ft', code: 'FEEDER', name: '피더', group: null, role: 'feeder', displayColor: null, fieldTemplate: null },
  ] }),
}));
