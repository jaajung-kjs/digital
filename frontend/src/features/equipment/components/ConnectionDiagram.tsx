...
import { CABLE_TYPES, CABLE_BADGE_CLASSES, buildCableTypesFromCategories } from '../../../types/connection';
import type { RoomConnection, MaterialCableTypeMeta } from '../../../types/connection';
...
export function ConnectionDiagram({
  ...
  const cableCategories = useMemo(
    () => cableCategoryData ? buildCableTypesFromCategories(cableCategoryData) : [],
    [cableCategoryData]
  );
  ...
  const handleStartCreation = (opt: MaterialCableTypeMeta) => {
    startCreation(equipmentId, opt.value, opt.materialCategoryId, opt.materialCategoryCode);
            ...
            >
              ...
              <div className="p-2">
                {cableCategories.length > 0 ? (
                  cableCategories.map((opt) => (
                    <button
                      key={opt.materialCategoryId}
                      onClick={() => handleStartCreation(opt)}
                  ...
                  CABLE_TYPES.map((opt) => (
                    <button
                      key={opt.value}
                      onClick={() => { startCreation(equipmentId, opt.value, '', ''); setShowCableSelector(false); }}
                      className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 transition-colors"
                    >
                      <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: opt.color }} />
                      <span className="text-sm font-medium text-gray-700">{opt.label}</span>
