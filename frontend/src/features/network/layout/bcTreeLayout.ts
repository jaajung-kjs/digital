...
export function computeLayoutBCTree(input: BCTreeLayoutInput): Map<string, { x: number; y: number }> {
  ...
  function placeRing(ring: TraceRing, anchorGroupId: string | null, anchorAngle: number) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      // ringCount>1 일 때 같은 gap(idx%ringCount)에 bridge 가 여러 개 와도 그 안에서 (k+1)/(m+1)
      // 로 균등 분산 → *정확히 겹치는*(modulo wrap) 문제 제거. gap 당 1개면 frac=0.5 라
      // 기존((g+0.5)*ringStep)과 완전히 동일 — 정상(≤1/gap) 케이스 동작 불변.
