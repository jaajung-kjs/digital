...
describe('buildFeederInput — role=IN 공급', () => {
  ...
  it('IN 케이블 1개 해소(공급원명 + 용량/개폐 포함, specParams 비면 빈 문자열, categoryName 없으면 규격 빈 문자열/categoryId null)', () => {
    const r = buildFeederInput({ id: 'f1' }, [inCable] as never, new Map([['srcEq', '주변압기']]));
    expect(r).toEqual({ cableId: 'in1', sourceAssetId: 'srcEq', sourceName: '주변압기', capacity: '', switchState: '', spec: '', categoryId: null });
  });
