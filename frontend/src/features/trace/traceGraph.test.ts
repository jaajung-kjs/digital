...
type AInput = {
  ...
  connectionKind?: 'conduit' | 'distributor' | null; code?: string | null; role?: string; slotIndex?: number | null;
};
const roleFor = (o: AInput): string =>
  o.role ?? (o.connectionKind === 'conduit' ? 'slot'
    : o.connectionKind === 'distributor' ? 'feeder'
    : o.code === 'OFD' ? 'ofd' : o.code === 'DIST' ? 'panel' : 'device');
const A = (o: AInput): Asset => ({
  ...
  assetType: { code: o.code ?? null, connectionKind: o.connectionKind ?? null, role: roleFor(o) },
} as unknown as Asset);
