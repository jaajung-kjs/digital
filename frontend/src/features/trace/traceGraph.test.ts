...
type AInput = {
  ...
  connectionKind?: 'conduit' | 'distributor' | null; code?: string | null; slotIndex?: number | null;
};
const A = (o: AInput): Asset => ({
  ...
  assetType: { code: o.code ?? null, connectionKind: o.connectionKind ?? null },
} as unknown as Asset);

// 원주 slotA ──OPGW── 홍천 slotB. eqA─OUT#5─slotA ; eqB─OUT#5─slotB ; eqC─OUT#6─slotB
const assets = [
  A({ id: 'slotA', name: 'OFD', substationId: 'subW', parentAssetId: 'ofdW', connectionKind: 'conduit', code: 'OFD-SLOT' }),
  A({ id: 'slotB', name: 'OFD', substationId: 'subH', parentAssetId: 'ofdH', connectionKind: 'conduit', code: 'OFD-SLOT' }),
...
describe('buildTraceGraph 변전소명 해소(org 트리 맵)', () => {
  it('자산 변전소명을 substationNames 로 해소', () => {
    const g = buildTraceGraph({
      assets: [A({ id: 'a1', substationId: 'sub-A', connectionKind: 'conduit', code: 'OFD-SLOT', name: '새슬롯' })],
  ...
  it('타 본부→강원 라벨 붕괴 회귀(Bug1): 자국·대국 변전소명 모두 해소', () => {
    const g = buildTraceGraph({
      assets: [
        A({ id: 'slotB', substationId: 'sub-GW', parentAssetId: 'ofdGW', connectionKind: 'conduit', code: 'OFD-SLOT' }),
        A({ id: 'ofdA', substationId: 'sub-B', name: 'OFD', code: 'OFD' }),
        A({ id: 'slotA', substationId: 'sub-B', name: '(구)춘천S/S', parentAssetId: 'ofdA', connectionKind: 'conduit', code: 'OFD-SLOT' }),
      ],
...
describe('ofdAssets / equipmentInSubstation (staged 가 저장 전에도 후보에 보임)', () => {
  ...
  it('ofdAssets: 스테이징 OFD(code 없이 placementKind=OFD)도 열거 — 저장 전 경로슬롯 추가 가능', () => {
    const g = buildTraceGraph({
      assets: [
        // 에디터로 막 만든 OFD: assetType.code 없음, placementKind 만 있음.
        { id: 'ofdStaged', name: 'OFD', substationId: 'sub-B', parentAssetId: null, slotIndex: null, assetType: { placementKind: 'OFD' } },
      ],
      cables: [],
  ...
  it('equipmentInSubstation: 스테이징 OFD(placementKind)도 후보에서 제외', () => {
    const g = buildTraceGraph({
      assets: [
        { id: 'eqNEW', substationId: 'sub-B', name: '통합단말', parentAssetId: null, slotIndex: null, assetType: { code: 'OPT-COT' } },
        { id: 'ofdStaged', substationId: 'sub-B', name: 'OFD', parentAssetId: null, slotIndex: null, assetType: { placementKind: 'OFD' } },
      ],
      cables: [],
      substationNames: new Map([['sub-B', 'B변전소']]),
    });
  ...
  it('equipmentInSubstation: 설비만 포함, OFD·conduit 제외', () => {
    const g = buildTraceGraph({
      assets: [
        ...
        A({ id: 'slotB', substationId: 'sub-B', name: '슬롯', connectionKind: 'conduit', code: 'OFD-SLOT' }),
      ],
      cables: [],
      substationNames: new Map([['sub-B', 'B변전소']]),
    });
