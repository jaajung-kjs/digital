...
type AInput = {
  ...
  role?: string; slotIndex?: number | null;
};
const A = (o: AInput): Asset => ({
  ...
  assetType: { id: '', name: '', role: (o.role ?? 'device') as any },
} as unknown as Asset);

// 원주 slotA ──OPGW── 홍천 slotB. eqA─OUT#5─slotA ; eqB─OUT#5─slotB ; eqC─OUT#6─slotB
const assets = [
  A({ id: 'slotA', name: 'OFD', substationId: 'subW', parentAssetId: 'ofdW', role: 'slot' }),
  A({ id: 'slotB', name: 'OFD', substationId: 'subH', parentAssetId: 'ofdH', role: 'slot' }),
  A({ id: 'eqA', name: '광단말A', substationId: 'subW', role: 'device' }),
  A({ id: 'eqB', name: '광단말B', substationId: 'subH', role: 'device' }),
  A({ id: 'eqC', name: '광단말C', substationId: 'subH', role: 'device' }),
];
const cables = [
...
describe('buildTraceGraph 변전소명 해소(org 트리 맵)', () => {
  it('자산 변전소명을 substationNames 로 해소', () => {
    const g = buildTraceGraph({
      assets: [A({ id: 'a1', substationId: 'sub-A', role: 'slot', name: '새슬롯' })],
  ...
  it('타 본부→강원 라벨 붕괴 회귀(Bug1): 자국·대국 변전소명 모두 해소', () => {
    const g = buildTraceGraph({
      assets: [
        A({ id: 'slotB', substationId: 'sub-GW', parentAssetId: 'ofdGW', role: 'slot' }),
        A({ id: 'ofdA', substationId: 'sub-B', name: 'OFD', role: 'ofd' }),
        A({ id: 'slotA', substationId: 'sub-B', name: '(구)춘천S/S', parentAssetId: 'ofdA', role: 'slot' }),
      ],
...
describe('ofdAssets / assetsInSubstation (staged 가 저장 전에도 후보에 보임)', () => {
  it('ofdAssets: 모든 OFD 자산 열거(이름 해소 포함)', () => {
    const g = buildTraceGraph({
      assets: [
        A({ id: 'ofdGW', substationId: 'sub-GW', role: 'ofd' }),
        A({ id: 'ofdNEW', substationId: 'sub-B', name: 'OFD', role: 'ofd' }),
      ],
      cables: [],
      substationNames: new Map([['sub-B', 'B변전소'], ['sub-GW', '(구)춘천S/S']]),
    });
  ...
  it('ofdAssets: 스테이징 OFD(role=ofd)도 열거 — 저장 전 경로슬롯 추가 가능', () => {
    const g = buildTraceGraph({
      assets: [
        // 에디터로 막 만든 OFD: role='ofd' 만 있음(staged 생성이 role 부여).
  ...
  it('assetsInSubstation: 설비만 포함, OFD·conduit 제외', () => {
    const g = buildTraceGraph({
      assets: [
        ...
        A({ id: 'ofdB', substationId: 'sub-B', name: 'OFD', role: 'ofd' }),
        A({ id: 'slotB', substationId: 'sub-B', name: '슬롯', role: 'slot' }),
      ],
      cables: [],
      substationNames: new Map([['sub-B', 'B변전소']]),
    });
