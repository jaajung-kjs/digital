...
// 를 갖는지 검증한다. CircuitPicker 가 고른 feeder id 는 interaction store 의
// innerAssetId 채널로 흐르고, CableSpecModal 이 이를 단일 assetId 로 stage 한다.
...
describe('CableSpecModal — 피더 케이블 endpoint = 피더 asset id', () => {
  beforeEach(() => {
  ...
  it('통신랙 → 전원 계통(F1) 케이블 → targetAssetId = F1 (단일 assetId)', () => {
    ...
    // source = 통신랙(설비), target = 피더 asset (picker 가 innerAssetId 채널로 feeder id 전달).
    const ix = useInteractionStore.getState();
    ix.cableSetSource('rack-1', { x: 0, y: 0 }, {});
    ix.cableSetTarget('panel-1', { x: 10, y: 10 }, { innerAssetId: FEEDER_ID });
