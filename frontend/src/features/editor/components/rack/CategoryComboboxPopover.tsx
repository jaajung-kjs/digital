...
/**
 * 랙 슬롯에 모듈을 배치할 때 뜨는 종류 선택 팝오버.
 * **선택 전용** — 종류(=AssetType) 생성/수정/삭제는 자산관리(카탈로그 워킹카피)에서만 한다.
 */
          ...
          {active.map((c) => (
            <li key={c.id}>
              <button
                type="button"
                onClick={() => onPick(c)}
                className="w-full min-w-0 flex items-center gap-2 px-3 py-1.5 text-sm text-left hover:bg-surface-2"
              >
