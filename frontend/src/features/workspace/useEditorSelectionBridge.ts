 ...
 * - 에디터 → 공유: detailAssetId 변화 관찰 → setSelectedAssetId
...
export function useEditorSelectionBridge(
  ...
  useEffect(() => {
    if (!active) return;
    prevEditorId.current = useEditorStore.getState().detailAssetId;
    const unsub = useEditorStore.subscribe((s) => {
      const id = s.detailAssetId;
  ...
  useEffect(() => {
    ...
    if (ed.detailAssetId === selectedAssetId) return; // 루프 가드
