 ...
 */
export function updateRackSourcePreset(rackAssetId: string, presetId: string | null): void {
  const store = useSubstationWorkingCopy.getState();
  const rackAsset = store.effectiveAssets().find((a) => a.id === rackAssetId);
  if (!rackAsset) return;
  store.stageAssetUpdate(rackAssetId, { sourcePresetId: presetId ?? null });
}
